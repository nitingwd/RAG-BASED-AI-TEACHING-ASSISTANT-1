...
def auth_ui():
    cookies = None
    if COOKIE_OK:
        cookies = EncryptedCookieManager(prefix="rag_white_red_", password="kadiya_naresh_white_red")
   ...
   .stApp {background: white!important;}
    ...
    div[data-testid="stButton"] > button {background: #FF0000!important; color: black!important; border: 2px solid #CC0000!important; border-radius: 12px!important; font-weight: 700!important;}
    </style>
    """, unsafe_allow_html=True)
    _, col, _ = st.columns([1,2,1])
    with col:
        st.markdown("""<div style="background:white; padding:36px; border-radius:20px; text-align:center; border:2px solid black;"><h2 style="color:black;">RAG Based AI Teaching Assistant</h2></div><br>""", unsafe_allow_html=True)
...
st.markdown("""
<style>
/* BACKGROUND SAB JAGAH WHITE */
.stApp {background: white!important; background-color: white!important;}
.main {background: white!important;}
.block-container {background: white!important; background-color: white!important; border: none!important; box-shadow: none!important;}
section[data-testid="stSidebar"] {background: white!important; background-color: white!important; border-right: 2px solid black!important;}
header[data-testid="stHeader"] {background: white!important;}
div[data-testid="stToolbar"] {background: white!important;}

/* FONT SAARE BLACK */
h1,h2,h3,h4,h5,h6 {color: black!important;}
p, span, label, div, li, a {color: black!important;}
div[data-testid="stMarkdownContainer"] * {color: black!important;}
.stMarkdown {color: black!important;}
label p {color: black!important;}

/* BUTTON SAARE RED + FONT BLACK */
...
div[data-testid="stButton"] > button * {color: black!important;}

/* FILE UPLOADER WHITE BG, BUTTON RED */
div[data-testid="stFileUploader"] {background: white!important; border: 2px solid black!important; border-radius: 12px!important;}
div[data-testid="stFileUploaderDropzone"] {background: white!important; border: 2px dashed black!important;}
div[data-testid="stFileUploader"] button {background: red!important; color: black!important; border: 2px solid black!important;}
...
/* RADIO, SLIDER BLACK */
div[data-testid="stRadio"] label {background: white!important; border: 1px solid black!important; color: black!important;}
div[data-testid="stSlider"] * {color: black!important;}

/* EXPANDER WHITE */
div[data-testid="stExpander"] {background: white!important; border: 2px solid black!important;}
div[data-testid="stVerticalBlockBorderWrapper"] {background: white!important; border: 2px solid black!important;}

/* AUDIO INPUT WHITE */
div[data-testid="stAudioInput"] {background: white!important; border: 2px solid black!important;}
div[data-testid="stAudioInput"] button {background: red!important; color: black!important; border: 2px solid black!important;}
...
.dev-badge {
    ...
    border: 2px solid black; z-index: 999;
}
</style>
...
def play_audio_block(text, lang, key):
    if not text: return
    if st.button(f"Play {lang}", key=f"play_{key}", use_container_width=True):
...
with st.sidebar:
    ...
    if user['photo'] and os.path.exists(user['photo']):
        try:
            with open(user['photo'], "rb") as f:
                ...
                photo_html = f'<img src="data:image/{ext};base64,{b64}" style="width:85px; height:85px; border-radius:50%; border:2px solid black; display:block; margin:0 auto; object-fit:cover;">'
        except:
            photo_html = f'<div style="width:85px; height:85px; border-radius:50%; background:red; display:flex; align-items:center; justify-content:center; margin:0 auto; color:black; font-size:32px; font-weight:800; border:2px solid black;">{user["name"][0].upper()}</div>'
    else:
        photo_html = f'<div style="width:85px; height:85px; border-radius:50%; background:red; display:flex; align-items:center; justify-content:center; margin:0 auto; color:black; font-size:32px; font-weight:800; border:2px solid black;">{user["name"][0].upper()}</div>'
    st.markdown(f"""<div style="background:white; border:2px solid black; border-radius:16px; padding:16px; text-align:center;">{photo_html}<h4 style="margin:12px 0 2px 0; color:black;">{user['name']}</h4><p style="margin:0; color:black; font-size:12px; font-weight:600;">@{user['username']}</p></div>""", unsafe_allow_html=True)
    st.write("")
    c1, c2 = st.columns(2)
    with c1:
    ...
    if st.button("Upload & Process", use_container_width=True, key="upload"):
        if uploaded_files:
...
st.markdown(f"""<div style="background:white; border:3px solid black; border-radius:20px; padding:20px 28px;"><h1 style="margin:0; color:black;">Welcome back, {user['name'].split()[0]}!</h1><h3 style="margin:6px 0 0 0; color:black;">RAG Based AI Teaching Assistant</h3></div><br>""", unsafe_allow_html=True)
...
with c2[2]:
    if st.button("Podcast", use_container_width=True, key="dash_pod"): st.session_state.active = "Podcast"; st.rerun()
with c2[3]:
    if st.button("Img->PDF", use_container_width=True, key="dash_img"): st.session_state.active = "Image2PDF"; st.rerun()
with c2[4]:
...
if active == "ChatView":
    ...
    with col1:
        ...
        else: st.markdown(f"<div style='width:180px; height:180px; border-radius:50%; background:red; display:flex; align-items:center; justify-content:center; font-size:60px; color:black; border:2px solid black;'>{user['name'][0].upper()}</div>", unsafe_allow_html=True)
    ...
    with col2:
        ...
        if st.button("Update Profile", use_container_width=True, key="upd_prof"):
    ...
    if st.button("Ask Question", use_container_width=True, key="ask_q") and q:
    ...
    if st.button(f"Generate in {lang}", use_container_width=True, key="imp_gen"):
    ...
    if st.button(f"Summary in {lang}", use_container_width=True, key="sum_gen"):
    ...
    if st.button("Create Story", use_container_width=True, key="story_gen") and tp:
    ...
    if st.button("Full IoT Guide Generate", use_container_width=True, key="proj_gen") and idea:
    ...
    if "last_proj" in st.session_state:
        ...
        st.download_button("TXT", st.session_state.last_proj, file_name="Full_IoT_Guide.txt", mime="text/plain", use_container_width=True)
elif active == "Podcast":
    tp = universal_input("pod", f"Topic {lang}")
    if st.button("Create Podcast", use_container_width=True, key="pod_gen") and tp:
    ...
    if uploaded_images:
        ...
        c1, c2 = st.columns(2)
        with c1:
            if st.button("PDF", use_container_width=True, key="pdf_btn"): pdf_data = images_to_pdf(uploaded_images); st.session_state.pdf_ready = pdf_data
        with c2:
            if st.button("DOCX", use_container_width=True, key="docx_btn"): docx_data = images_to_docx(uploaded_images); st.session_state.docx_ready = docx_data
        if "pdf_ready" in st.session_state: st.download_button("PDF", st.session_state.pdf_ready, file_name="RAG_Images.pdf", mime="application/pdf", use_container_width=True, key="dl_pdf")
        if "docx_ready" in st.session_state: st.download_button("DOCX", st.session_state.docx_ready, file_name="RAG_Images.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True, key="dl_docx")
elif active == "Quiz":
    ...
    if st.button("Start Viva", use_container_width=True, key="viva_start") and topic:
    ...
    if st.button("Create PPT", use_container_width=True, key="ppt_create") and topic:
        ppt_path = create_ppt_file(topic, pages, language=lang)
        with open(ppt_path, "rb") as f: st.download_button("Download PPT", f, file_name=f"{topic}_{lang}.pptx", key="ppt_dl")
