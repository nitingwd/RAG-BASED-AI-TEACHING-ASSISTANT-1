...
def auth_ui():
    cookies = None
    if COOKIE_OK:
        cookies = EncryptedCookieManager(prefix="rag_v20_", password="kadiya_naresh_2024_secure_key_v20")
   ...
   .pill {display:inline-block; background:linear-gradient(135deg,#EEF2FF,#E0E7FF); color:#6366F1; padding:6px 12px; border-radius:20px; font-size:11px; font-weight:700; margin:3px; border:1px solid #C7D2FE;}
    div[data-testid="stTabs"] button {border-radius:12px; font-weight:600;}
   .stButton>button {border-radius:12px; height:48px; font-weight:700; transition:all 0.2s;}
   .stButton>button:hover {transform:translateY(-1px); box-shadow:0 10px 24px rgba(99,102,241,0.3);}
    </style>
    ...
    with col:
        ...
        <div class="login-card">
            ...
            <div style="margin-top:14px;">
                <span class="pill">💬 Q&A</span><span class="pill">🎤 VIVA</span><span class="pill">📝 QUIZ</span><span class="pill">🔊 AUDIO</span><span class="pill">📄 SUMMARY</span>
            </div>
        </div><br>
        """, unsafe_allow_html=True)
...
.hero-pro::after {content:''; position:absolute; bottom:-50%; left:-10%; width:300px; height:300px; background: radial-gradient(circle, rgba(255,255,255,0.15) 0%, transparent 70%); border-radius:50%;}
...
.feature-btn {background:white; border:1px solid #E0E7FF; border-radius:14px; padding:12px 8px; text-align:center; font-weight:600; font-size:13px; transition:all 0.2s; cursor:pointer;}
.feature-btn:hover {border-color:#6366F1; box-shadow:0 8px 20px rgba(99,102,241,0.12); transform:translateY(-2px);}
.dev-badge {position: fixed; bottom: 14px; right: 14px; background: #111827; color: white; padding: 7px 12px; border-radius: 20px; font-size: 10px; z-index: 999; letter-spacing:0.4px; font-weight:600;}
.stButton>button {border-radius:12px; height:46px; font-weight:700; transition:all 0.2s;}
.stButton>button:hover {transform:translateY(-1px);}
</style>
<div class="dev-badge">V20 ULTIMATE PRODUCTION | KADIYA NARESH</div>
""", unsafe_allow_html=True)
    ...
    if st.button(f"▶️ {lang} me Suno", key=f"play_{key}", type="primary", use_container_width=True):
        with st.spinner("Audio bana raha hu..."):
            ...
            if ap and os.path.exists(ap) and os.path.getsize(ap)>500:
                ...
                st.download_button("⬇️ MP3 Download", audio_bytes, file_name=f"{key}.mp3", mime="audio/mp3", key=f"dl_{key}")

# ================= SIDEBAR - PRODUCTION WITH CIRCLE PHOTO =================
with st.sidebar:
    # CIRCLE PHOTO LOGIC
    photo_html = ""
    ...
    if st.button("🚀 Upload & Process", type="primary", use_container_width=True):
        if uploaded_files:
            ...
            save_conversation(user['id'], f"Uploaded {len(uploaded_files)} files", "Files processed", "Upload")
    ...
    st.caption(f"Chats: {len(convs)} | Lang: {selected_language}")
...
<div class="hero-pro">
    <div style="position:relative; z-index:2;">
        ...
        <h2 style="margin:4px 0 0 0; font-family:Space Grotesk; font-size:18px; font-weight:600; opacity:0.95;">RAG Based AI Teaching Assistant</h2>
        <p style="margin:10px 0 0 0; opacity:0.9; font-size:13px; line-height:1.4;">🔒 Persistent login ON • 🖼️ Circle profile • {len(convs)} chats saved • Production ready UI</p>
    </div>
</div><br>
""", unsafe_allow_html=True)
...
# ================= UNIQUE COLORFUL FEATURE GRID =================
st.markdown("#### ⚡ Production Dashboard - All Features")
def fbtn(label, act, color):
    if st.button(label, use_container_width=True, key=f"fb_{act}_{label[:2]}"):
        st.session_state.active=act; st.rerun()

c = st.columns(6)
with c[0]: fbtn("💬 Ask Q&A", "Ask", "#6366F1")
with c[1]: fbtn("⭐ Important", "Important", "#F59E0B")
with c[2]: fbtn("🔧 Project", "Projects", "#10B981")
with c[3]: fbtn("📝 Quiz", "Quiz", "#EF4444")
with c[4]: fbtn("🎤 Viva", "Viva", "#8B5CF6")
with c[5]: fbtn("📊 PPT Maker", "PPT", "#06B6D4")
c2 = st.columns(6)
with c2[0]: fbtn("📄 Summary", "Summary", "#6366F1")
with c2[1]: fbtn("📖 Story Mode", "Story", "#EC4899")
with c2[2]: fbtn("🎙️ Podcast", "Podcast", "#F59E0B")
with c2[3]: fbtn("🖼️ Img→PDF", "Image2PDF", "#10B981")
with c2[4]: fbtn("👤 Profile", "Profile", "#8B5CF6")
with c2[5]: fbtn("🕘 History", "History", "#06B6D4")
st.divider()

active = st.session_state.active
lang = st.session_state.get('selected_language','Hinglish')
st.markdown(f"### ▶ {active} Mode • 🌐 {lang}")

# ================= MODES =================
if active=="ChatView":
    conv = st.session_state.get("selected_conv")
    if not conv: st.info("Sidebar se koi chat select karo")
    else:
        ...
        if st.button("⬅️ Back to Ask"): st.session_state.active="Ask"; st.rerun()
elif active=="Profile":
    col1,col2 = st.columns([1,2])
    with col1:
        if user['photo'] and os.path.exists(user['photo']):
            st.image(user['photo'], width=200)
            st.markdown("<p style='text-align:center; color:#6366F1; font-size:12px; font-weight:600;'>✓ Circle me dikhega sidebar me</p>", unsafe_allow_html=True)
        else:
    ...
    with col2:
        ...
        new_bio = st.text_area("Bio", value=user['bio'] if user['bio'] else "", placeholder="I am a student learning AI...")
        if st.button("💾 Update Profile", type="primary", use_container_width=True):
            update_profile(user['id'], new_name, new_email, new_bio, new_photo); st.success("Profile Updated! Ab sidebar me circle photo dekho"); st.rerun()
    st.divider()
    m1,m2,m3 = st.columns(3)
    m1.metric("Username", f"@{user['username']}")
    m2.metric("Total Chats", len(get_user_conversations(user['id'])))
    m3.metric("Member Since", user['id'])
elif active=="History":
    convs = get_user_conversations(user['id'])
    st.metric("Total Conversations", len(convs))
    for cid, q, a, mode, ts in convs:
        with st.expander(f"[{mode}] {q[:70]} • {ts[:16]}"):
            st.markdown(f"**Q:** {q}"); st.divider(); st.markdown(a)
elif active=="Ask":
    mode = st.radio("Answer Style:", ["Normal - Direct Answer","Socratic - Question se samjhao"], horizontal=True); sel = "socratic" if "Socratic" in mode else "normal"
    ...
    if st.button(f"⭐ Generate Important Qs in {lang}", type="primary", use_container_width=True):
    ...
    if st.button(f"📄 Generate Summary in {lang}", type="primary", use_container_width=True):
    ...
    tp=universal_input("story",f"Topic in {lang} - e.g. Stack, Recursion")
    if st.button("🎬 Create Story + Audio", type="primary", use_container_width=True) and tp:
    ...
    idea=universal_input("proj",f"Project idea ({lang}) - e.g. AI Chatbot")
    bud=st.selectbox("Budget", ["low (under ₹1500)","medium (₹1500-5000)","high (₹5000+)"], key="bud")
    if st.button("🔧 Generate Project Guide", type="primary", use_container_width=True) and idea:
    ...
    tp=universal_input("pod",f"Podcast topic in {lang}")
    if st.button("🎙️ Create Podcast Script", type="primary", use_container_width=True) and tp:
    ...
    st.markdown("### 🖼️ Image to PDF / DOCX - Production Tool")
    uploaded_images = st.file_uploader("Images Upload (JPG, PNG)", type=["jpg","jpeg","png"], accept_multiple_files=True, key="img_upload")
    if uploaded_images:
        ...
        if "pdf_ready" in st.session_state:
            st.download_button("⬇️ Download PDF", st.session_state.pdf_ready, file_name=f"RAG_Teaching_{len(uploaded_images)}_Images.pdf", mime="application/pdf", use_container_width=True)
        if "docx_ready" in st.session_state:
            st.download_button("⬇️ Download DOCX", st.session_state.docx_ready, file_name=f"RAG_Teaching_{len(uploaded_images)}_Images.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True)
    else: st.info("Images upload karo - Multiple select kar sakte ho")
elif active=="Quiz":
    n = st.number_input("Kitne Questions?", 3, 15, 5)
        ...
        for i, qd in enumerate(st.session_state.quiz_data):
            with st.container(border=True):
                ...
                if st.button(f"✓ Check Q{i+1}", key=f"chk_{i}"):
                    ...
                    st.session_state.quiz_results.append({"is_correct": ok}); st.success(fb) if ok else st.error(fb); st.info(qd.get('explanation',''))
        if st.session_state.quiz_results:
            st.divider()
            correct = sum(1 for r in st.session_state.quiz_results if r['is_correct'])
            st.metric("Score", f"{correct}/{len(st.session_state.quiz_results)} - {correct/len(st.session_state.quiz_results)*100:.1f}%")
            if st.button("🔄 Reset Quiz"): st.session_state.quiz_data=None; st.session_state.quiz_results=[]; st.rerun()
elif active=="Viva":
    topic=universal_input("viva_topic",f"Viva topic ({lang}) - e.g. DBMS")
    num=st.slider("Kitne Questions?", 3, 20, 5)
    ...
    if st.session_state.viva_qs:
        ...
        if idx < len(st.session_state.viva_qs):
            ...
            if st.button("✅ Submit Answer", key=f"v_sub_{idx}", use_container_width=True) and user_ans:
                ...
                st.success(f"✅ {res.get('feedback','')}") if is_correct else st.error(f"❌ {res.get('feedback','')} - Sahi: {curr['a']}")
                st.session_state.viva_idx+=1; st.rerun()
        else:
            st.balloons(); st.success(f"Viva Complete! Score: {sum(1 for s in st.session_state.viva_score if s['is_correct'])}/{len(st.session_state.viva_score)}")
            if st.button("🔄 New Viva"): st.session_state.viva_qs=[]; st.rerun()
elif active=="PPT":
    topic=universal_input("ppt",f"PPT Topic in {lang} - e.g. AI in Education")
    pages=st.slider("Kitni Slides?", 5, 25, 10)
    if st.button("📊 Create PPT", type="primary", use_container_width=True) and topic:
        ...
        with open(ppt_path,"rb") as f:
            st.download_button("⬇️ Download PPT", f, file_name=f"{topic}_{lang}.pptx", use_container_width=True, mime="application/vnd.openxmlformats-officedocument.presentationml.presentation")
        st.success("PPT Ban gaya!")
